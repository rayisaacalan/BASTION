...
save(aral_speed, file = "aral_speed_results.Rda")
```


```{r aral-speed-xtable, results='asis'}
datapath = paste0(getwd(), "/aral_speed_results.Rda")
attach(what = datapath, name = "precomputed")
speed_table1 = xtable(summary(aral_speed), caption = "Speed comparison (seconds) between the different native R implementations of the BAST model.")
detach(precomputed)
print(speed_table1, type = "html", comment = FALSE)
```



Unsurprisingly, the speed leaves much to be desired with even the fastest possible fitting time coming in at almost 7 minutes on a relatively small data set. Further, the original code from the paper is more performant at the cost of being more specific to this particular setup. Let's take a look at the performance after rewriting in C++ using Rcpp:
...
```{r, eval = FALSE}
library(RcppClock)
load("aral_cpp_speed_profile.Rda")
cpp_df = data.frame(process = summary(BASTIONfit_C_timings)$ticker,
                    time_spent_s = summary(BASTIONfit_C_timings)$mean * summary(BASTIONfit_C_timings)$neval * 0.000001)
cpp_df
```
...
```{r test-cpp-speed-one, eval = FALSE}
test_cpp_speed = microbenchmark(
  bastionfit_C = BASTIONfit_C(Y_std, aral_graph, init_val, hyperpar, MCMC, BURNIN, THIN, seed = 12345),
  times = 1
)
```
...
test_coords = data_full[x^2 + y^2 >= 16, ]
test_graph = constructGraph(test_coords, 4)
...
z = 1.2^(test_coords$x) + 
  (test_coords$y)^2 - 
  1000*(test_coords$x > test_coords$y) + 
  4*test_coords$x*test_coords$y # response

# ggraph(test_graph, layout = test_coords) + 
#   geom_edge_link0(edge_alpha = 0.1) + 
#   geom_node_point(aes(color = z)) +
#   scale_color_viridis()
...
q = BASTIONfit_C(z, test_graph, init_val, hyperpar, MCMC, BURNIN, THIN, seed = 12345)
```
